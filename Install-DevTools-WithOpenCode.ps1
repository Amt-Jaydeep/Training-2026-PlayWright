...
<#
.SYNOPSIS
    Auto Install Dev Tools: uv, Git, Playwright, Zed, Node.js, OpenCode
.DESCRIPTION
    One-click install script for training computer with OpenCode AI
    Must run as Administrator
.NOTES
    Right-click -> Run as Administrator PowerShell
    Or run: .\Install-DevTools-WithOpenCode.ps1
#>
...
Write-Host "   Dev Tools Auto Install (With OpenCode)   " -ForegroundColor Cyan
Write-Host "   uv / Git / Playwright / Zed / Node.js / OpenCode   " -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
# 3. Install Node.js
# ============================================================
Write-Log "========== [3/6] Install Node.js =========="

if (Test-CommandExists "node") {
    $nodeVersion = node --version 2>$null
    Write-Log "Node.js already installed: $nodeVersion" "OK"
} else {
    ...
    if (-not $installed) {
        ...
        try {
            ...
            Write-Log "Node.js install failed, please install manually" "ERROR"
        }
    }
...
# 4. Install OpenCode
# ============================================================
Write-Log "========== [4/6] Install OpenCode =========="

if (Test-CommandExists "opencode") {
    $ocVersion = opencode --version 2>$null
    Write-Log "OpenCode already installed: $ocVersion" "OK"
} else {
    if (Test-CommandExists "npm") {
        Write-Log "Installing OpenCode via npm..."
        try {
            ...
            if (Test-CommandExists "opencode") {
                $ocVersion = opencode --version 2>$null
                Write-Log "OpenCode installed: $ocVersion" "OK"
            } else {
                Write-Log "OpenCode installed but not in PATH, please restart terminal" "WARN"
            }
        } catch {
            Write-Log "OpenCode install failed: $_" "ERROR"
        }
    } else {
...
# ============================================================
Write-Log "========== [5/6] Install Playwright =========="
...
if (Test-CommandExists "node") {
    $results += @{ Tool = "Node.js"; Status = "V"; Version = (node --version 2>$null) }
} else {
    $results += @{ Tool = "Node.js"; Status = "X"; Version = "Not installed" }
}

if (Test-CommandExists "opencode") {
    $results += @{ Tool = "OpenCode"; Status = "V"; Version = (opencode --version 2>$null) }
} else {
    $results += @{ Tool = "OpenCode"; Status = "X"; Version = "Not installed" }
}
